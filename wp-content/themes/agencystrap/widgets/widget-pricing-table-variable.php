 ...
 */
function agencystrap_pricing_table_variable_widget() {
	register_widget( 'Pricing_Table_Variable_Options' );
}
add_action( 'widgets_init', 'agencystrap_pricing_table_variable_widget' );
...
class Pricing_Table_Variable_Options extends WP_Widget {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_agencystrap_check_for_download_price_variations', array( $this, 'agencystrap_check_for_download_price_variations' ) );
		...
		$widget_ops = array(
			...
			'description' => __( 'Displays a pricing table using the variable pricing options of the selected download.', 'agencystrap' ),
		);
		...
		parent::__construct( 'pricing-table-variable-options', __( 'agencystrap: Variable Pricing Table', 'agencystrap' ), $widget_ops, $control_ops );
 ...
 ?>
		...
		<section class="pricing-section <?php if( $featured ) { echo 'featured-price'; } ?>">
				...
				<?php foreach( $prices as $key => $price ) : ?>
					<div itemscope class="pricing-table <?php if( $key == $featured ) { echo 'featured'; } ?>">
						...
						<div class="pricing-table-features">
							<div class="download-details download-aside">
								<div class="download-features" itemprop="itemCondition">
									...
									<?php
										...
										$list_items = agencystrap_edd_download_details_list_items( $download_id, $key );
										...
										$list_item_all_prices = agencystrap_edd_download_details_list_items( $download_id, 'all' );
							...
							<a class="button" href="<?php echo edd_get_agencystrap_uri(); ?>?edd_action=add_to_cart&amp;download_id=<?php echo absint( $download_id ); ?>&amp;edd_options%5Bprice_id%5D=<?php echo absint( $key ); ?>" title="<?php echo esc_attr( $price['name'] ); ?>">
								<?php _e( 'Buy Now', 'agencystrap' ); ?>
							</a>
						</div>
					</div>
				<?php endforeach; ?>
	...
	function form( $instance ) {
			...
			<div class="variable-price-table-widget-column-box variable-price-table-widget-column-box-top">
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_name( 'download' ) ); ?>">
						<?php printf( __( 'Select a %1$s: ', 'agencystrap' ), edd_get_label_singular() ); ?>
					</label>

					<select class="agencystrap_download_select" id="<?php echo esc_attr( $this->get_field_id( 'download' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'download' ) ); ?>">
						<option value="none"><?php _e( 'None', 'agencystrap' ); ?></option>
				...
				<p class="price-variation-wrapper">
					<label for="<?php echo esc_attr( $this->get_field_id( 'price_variation' ) ); ?>">
						<?php _e( 'Select a price option to feature:', 'agencystrap' ); ?><br>
					</label>
					<?php if( ! $instance['price_variation'] || empty( $instance['price_variation'] )  || 'none' == $instance['price_variation'] ) : ?>

						<select class="agencystrap_price_options_select" id="<?php echo esc_attr( $this->get_field_id( 'price_variation' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'price_variation' ) ); ?>">
							<option value="none"><?php _e( 'None', 'agencystrap' ); ?></option>
						</select>
					<?php else : ?>
						<select class="agencystrap_price_options_select" id="<?php echo esc_attr( $this->get_field_id( 'price_variation' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'price_variation' ) ); ?>">
		...
		<script>
			jQuery(document).ready(function ($) {
				$(".agencystrap_download_select").on( 'change', function() {
					var $this = $(this), download_id = $this.val();

					if( parseInt(download_id) > 0 ) {
						var postData = {
							action: 'agencystrap_check_for_download_price_variations',
						...
						$.ajax({
							...
							success: function (response) {
								$('.agencystrap_price_options_select').remove();
	...
	function agencystrap_check_for_download_price_variations() {
		...
		if ( edd_has_variable_prices( $download_id ) ) {
			...
			$ajax_response = '<select class="agencystrap_price_options_select" name="' . esc_attr( $this->get_field_name( 'price_variation' ) ) .'">';
			$ajax_response.= '<option value="none">' . __( 'None', 'agencystrap' ) . '</option>';
			if ( $variable_prices ) {
					foreach ( $variable_prices as $key => $price ) {
